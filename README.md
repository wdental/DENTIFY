...
- **El odontograma se dibuja reutilizando el renderizador real**: `imprimir-f033.html` carga `public/js/odontograma.js` (que no ejecuta nada por sí solo — sin `DOMContentLoaded`, solo define funciones) y llama a `renderizarSvgOdontograma(elementoDestino)` (acepta un `<svg>` de destino opcional, por defecto el de la app interactiva) con `piezasVisibles` cargado desde cada versión y `modoEdicion = false`. Mismos símbolos, mismos colores, mismas correcciones visuales que en pantalla — cero duplicación de lógica de dibujo. La leyenda (sección K) reutiliza igual `construirLeyenda()`.
- **Odontograma inicial + actual lado a lado** (ajuste tras uso real): si el paciente ya tiene evoluciones (la versión activa dejó de ser la inicial), la sección H imprime **dos** diagramas — "Odontograma inicial" (línea base fija, siempre la misma) y "Odontograma actual (evolución)" (la versión activa) — para poder comparar lo encontrado en la primera consulta contra el estado presente sin perder ese registro visual. Si el paciente aún no tiene ninguna evolución, se imprime un solo diagrama a ancho completo, igual que antes. Ambos se renderizan llamando a `renderizarSvgOdontograma()` dos veces seguidas (una por versión) sobre dos `<svg>` distintos.
