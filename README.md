...
- **J. Índices CPO-ceo** — **fuente única** del CPO-ceo (ya no existe el panel lateral del odontograma). Muestra en **solo lectura por defecto** el autocálculo derivado del odontograma activo (`GET /api/odontograma/:pacienteId/cpo-sugerido`), que se refresca cada vez que se guarda una nueva versión del odontograma. Botón **"Ajustar manualmente"** habilita la edición de los campos; al guardar, el valor queda marcado como ajustado manualmente con quién y cuándo (`ajustado_por`/`ajustado_en` en `indices_cpo_json`). Botón **"Restaurar cálculo automático"** vuelve al valor derivado. Mientras se edita una versión del odontograma, un contador compacto de una sola línea ("CPO permanente C:x P:x O:x · ceo temporal c:x e:x o:x") acompaña al contador de hallazgos.
